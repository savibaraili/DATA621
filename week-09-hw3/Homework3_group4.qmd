...
subtitle: "Group 3"
author: "Zoran G., Natalie K., Sabina B., Woodelyne D., Zaneta P."
format:
  pdf:
    toc: false
...
Because `target` is a binary `0/1` outcome rather than a continuous response, correlation coefficients should be interpreted cautiously. Pearson correlation is still acceptable here when one variable is binary, so we used it as a familiar baseline. We also calculated Spearman correlation as a robustness check because it is rank-based and less sensitive to distributional shape. The main patterns were stable across both methods, which increased confidence that the strongest relationships were not artifacts of a single correlation choice.
...
The table below compares the three candidate models using the required training-set metrics.
...
One thing worth flagging in Models 1 and 2 is that `dis` and `medv` have positive coefficients, even though both showed negative correlations with `target` during exploration — meaning higher distance from employment centers and higher home values were associated with lower crime risk in the raw data. This sign flip is a consequence of multicollinearity. `nox` and `dis` are strongly negatively correlated: neighborhoods closer to employment centers tend to have higher pollution. When `nox` and `dis` are both in the model, `nox` absorbs most of the crime signal, and `dis` ends up with an adjusted coefficient that goes against its raw direction. The same mechanism affects `medv`. This is expected behavior in logistic regression with correlated predictors, not an error in the model. We are keeping Models 1 and 2 as comparison baselines precisely because they illustrate how raw correlations can be misleading once other variables are controlled for. We did not add a formal VIF table, but the pairwise correlation review was enough to show that some predictors were carrying overlapping information.
...
This also gives a direct way to interpret coefficients. A positive coefficient means that, holding the other variables fixed, an increase in that predictor increases the log-odds of being in the high-crime class; a negative coefficient lowers those log-odds. For nonlinear terms such as squared predictors, the interpretation is not constant across the entire range, but the sign still indicates whether the effect bends upward or downward as the predictor increases.

The signs of the main coefficients are also reasonable:

- `nox_sq` is strongly positive, indicating that as `nox` increases, the log-odds of high crime increase at an increasing rate.
...
Overall, the coefficients make sense as a group. The strongest predictors align with the exploratory analysis and with reasonable domain expectations about pollution, age of housing, transportation access, and neighborhood advantage. Where a coefficient looks counterintuitive in a simpler model, we keep it only when it improves fit and can be explained by the presence of correlated predictors rather than by a coding mistake.
...
One caution is that the final transformed logistic model produced `glm.fit` warnings that some fitted probabilities were numerically `0` or `1`. That suggests possible quasi-separation: the transformed predictors separate the classes very strongly for some training observations. We still keep the model because its predictive performance is best and the assignment is focused on logistic classification, but this warning means the coefficient magnitudes should be interpreted somewhat more cautiously than the classification metrics.
